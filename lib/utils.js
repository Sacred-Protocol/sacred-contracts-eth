...
async function setup ({ethSacredAbiPath, erc20SacredAbiPath, sacredProxyContract, withdrawCircuit, withdrawProvidingKeyFilePath}) {
  ...
  const ethSacredAbi = require(ethSacredAbiPath + '/ETHSacred.json')
  const erc20SacredAbi = require(erc20SacredAbiPath + '/ERC20Sacred.json')
  ...
  netkeys.forEach(netKey => {
    ...
    currencies.forEach(currency => {
      ...
      amounts.forEach(amount => {
        ...
        if(address) {
          contracts[key] = new ethers.Contract(address, currency === "eth" ? ethSacredAbi.abi : erc20SacredAbi.abi, wallet)
        }
      })
    })
  })
